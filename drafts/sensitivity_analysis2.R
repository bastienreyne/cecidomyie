...
N <- 10000
Matrice <- matrix(runif(N * 6 * 2), nrow = N)
...
mat_C1 <- cbind(mat_A[, 1], mat_B[, 2:6])
mat_C2 <- cbind(mat_B[, 1], mat_A[, 2], mat_B[, 3:6])
mat_C3 <- cbind(mat_B[, 1:2], mat_A[, 3], mat_B[, 4:6])
mat_C4 <- cbind(mat_B[, 1:3], mat_A[, 4], mat_B[, 5:6])
mat_C5 <- cbind(mat_B[, 1:4], mat_A[, 5], mat_B[, 6])
mat_C6 <- cbind(mat_B[, 1:5], mat_A[, 6])
...
to_plot <- data.frame(Paramètre = factor(rep(c("gamma", "pps", "muER",
                                               "muEH", "k", "mu_glob"), times = 6),
                                         levels = c("gamma", "pps", "muER",
                                                    "muEH", "k", "mu_glob")),
                      Valeur = c(S_ER1, S_ER2, S_ER3, S_ER4, S_ER5, S_ER6,
                                  ST_ER1, ST_ER2, ST_ER3, ST_ER4, ST_ER5, ST_ER6,
                                  S_PS1, S_PS2, S_PS3, S_PS4, S_PS5, S_PS6, 
                                  ST_PS1, ST_PS2, ST_PS3, ST_PS4, ST_PS5, ST_PS6,
                                  S_EH1, S_EH2, S_EH3, S_EH4, S_EH5, S_EH6, 
                                  ST_EH1, ST_EH2, ST_EH3, ST_EH4, ST_EH5, ST_EH6),
                      Indice = rep(c("Effet principal", "Effet total"),
                                   each = 6, times = 3),
                      Sol = factor(rep(c("Enherbement ras",
                                         "Paillage synthétique",
                                         "Enherbement haut"), each = 12),
...
to_plot %>% ggplot +
    ...
    scale_x_discrete(labels = c(expression(gamma), expression(p[m]),
                                ...
                                expression(k), expression(mu[global])))

...
larves <- function(jour, capacite.inflo, eggs, inflos, femelles){
...
femelles.endogene <- function(jour, proba.pupaison, duree.pupaison, larves, mu.sol){
...
dynamiques <- function(gamma, proba.migration, mu.ER, mu.EH, capacite.inflo, eggs, proba.pupaison, duree.pupaison, inflos){
    ...
    for (jour in 1:nb.jours){
        ...
        larves[jour,] <- larves(jour, as.numeric(capacite.inflo), as.numeric(eggs), inflos, femelles)
        
        femelles.endo[jour,] <- femelles.endogene(jour, as.numeric(proba.pupaison), round(duree.pupaison), larves, mu.sol)

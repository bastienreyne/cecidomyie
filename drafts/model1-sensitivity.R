...
y.est <- matrix(NA, nrow = nb, ncol = 3)
...
for (i in 1:nb) {
    ...
    ## Var(Y)
    ind_y <- sample(1:nb, 5, replace = TRUE)
    y.est[i, ] <- objectif(c(gammas[ind_y[1]], pms[ind_y[2]],
                             muers[ind_y[3]], muehs[ind_y[4]], ks[ind_y[5]]), my.mae)
}
...
save(SG, SPM, SMUER, SMUEH, SK, SGt, SPMt, SMUERt, SMUEHt, SKt, file="soboled4.Rdata")

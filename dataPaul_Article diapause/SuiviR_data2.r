...
# par ordre chronologique de janvier à décembre
...
plot(c(0.5:12.5),jour,ylim=c(550,800),xlim=c(0,13),axes=F,xlab="",ylab="",type="p",pch=8,cex=1.2)
par(new=T)
plot(c(0.5:12.5),jour,ylim=c(550,800),xlim=c(0,13),axes=F,xlab="",ylab="",type="b",pch=19,lty=10)
axis(4,line=3)
...
tete <- as.data.frame(cbind(pour=tapply(totlarviv,mois,sum),nb=tapply(nblarv,mois,sum)-tapply(paras,mois,sum)))
tete <- rbind(tete[1:4,],as.data.frame(cbind(pour=rep(0,3),nb=rep(0,3))),tete[5:8,],as.data.frame(cbind(pour=c(0),nb=c(0))),tete[9,])
tete$num <- (tete$pour/tete$nb)*(1-tete$pour/tete$nb)
tete$errstd <- sqrt(tete$num/tete$nb)
tete$mois <- row.names(tete)
...
par(mar=c(3, 4, 1, 6),cex=1.3,las=1)
barplot2(tete$pour/tete$nb,plot.ci=T,ci.u=(tete$pour/tete$nb)+2*tete$errstd,ci.l=(tete$pour/tete$nb)-2*tete$errstd,
        ylim=c(-.02,0.49),names.arg =c("Jun","Jul","Aug","Sep","","","","Jan","Feb","Mar","Apr","","Jun"),
        xlab="Month",ylab="Rate of diapause")
legend(0,0.4,c("Temperature","Day length"),lty=c(1,10),pch=c(1,8,9),bty="n")
j <- 0
for (i in c(1:4,8:11,13) )
{
j <- j+1
valeur <- c("d","d","c","d","ab","a","b","c","c")
xpos  <- seq(0.7,20,1.2)
ypos <- c(0.05706527,0.04825663,0.15232907,0.04407272,0.34179349,0.36357806,0.24927134,0.14346253,0.10760459) +0.015
...
axis(4,line=-1)
par(las=0)
mtext("Temperature (°c)", 4,adj=0.5,line=1,cex=1.4)
par(las=1)
# rajoute la duree du jour
par(new=T,las=0)
jour <- c(durjour[6:12],durjour[1:6])
...
plot(c(0.5:12.5),jour,ylim=c(550,800),xlim=c(0,13),axes=F,xlab="",ylab="",type="b",pch=19,lty=10)
axis(4,line=3)
mtext("Day length (minutes)", 4,adj=0.5,line=5,cex=1.4)
par(las=1)
...
par(mar=c(3, 4, 1, 6),cex=1.3,las=1, col.axis="black",col="black")
barplot2(tete$pour/tete$nb,plot.ci=T,ci.u=(tete$pour/tete$nb)+2*tete$errstd,ci.l=(tete$pour/tete$nb)-2*tete$errstd,
        ylim=c(-.02,0.49),names.arg =c("Jun","Jul","Aug","Sep","","","","Jan","Feb","Mar","Apr","","Jun"),
        xlab="Month",ylab="Rate of diapause",border="black",axes=F)
axis(2,line=0, col="black")
legend(0,0.4,c("Temperature","Day length"),lty=c(1,10),bty="n",col=c("blue","red"), text.col=c("blue","red"))
j <- 0
for (i in c(1:4,8:11,13) )
{
j <- j+1
...
yy <- tapply(temp.df$tempm,paste(temp.df$mois,temp.df$an),mean,na.rm=T)[c(9,7,1,17,15,13,4,6,5,12,3,11,10)]
plot(c(0.5:12.5),yy,ylim=c(17,26),xlim=c(0,13),axes=F,xlab="",ylab="",type="b", col="blue",pch="!")
axis(4,line=-1, col="blue")
par(las=0)
mtext("Temperature (°c)", 4,adj=0.5,line=1,cex=1.4, col="blue")
par(las=1)
# rajoute la duree du jour
par(new=T,las=0, col.axis="red")
jour <- c(durjour[6:12],durjour[1:6])
plot(c(0.5:12.5),jour,ylim=c(550,800),xlim=c(0,13),axes=F,xlab="",ylab="",type="p",pch=8,cex=1.2, col ="red")
par(new=T)
plot(c(0.5:12.5),jour,ylim=c(550,800),xlim=c(0,13),axes=F,xlab="",ylab="",type="b",pch=19,lty=10, col ="red")
axis(4,line=3, col ="red")
mtext("Day length (minutes)", 4,adj=0.5,line=5,cex=1.4, col ="red")
par(las=1)
